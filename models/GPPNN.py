...
class CoupleLayer(nn.Module):
    def __init__(self, channels, subnet, substructor, condition_length,  clamp=5.):
        super().__init__()
        ...
        self.subnet = subnet(1, channels//2)
...
class PanProcess(nn.Module):
    def __init__(self, in_size, out_size, relu_slope=0.1, use_HIN=True):
        ...
        self.conv1 = nn.Conv2d(in_size,out_size,1,1,0)
        self.CDC = cdcconv(out_size,out_size)
    ...
    def forward(self, x):
        out = self.conv1(x)
        # out = torch.cat([self.conv1(x),self.CDC(x)],1)
        out = self.Res(out)+self.CDC(out)
...
class MultiscaleDense(nn.Module):
    def __init__(self,channel_in, channel_out, init):
        ...
        self.pool = nn.Sequential(
                ...
                nn.Conv2d(3*channel_out, 3*channel_out // 4, 1, padding=0, bias=True),
                nn.ReLU(inplace=True),
                nn.Conv2d(3*channel_out // 4, 3*channel_out, 1, padding=0, bias=True),
                nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        xout = self.fuse(xout*self.pool(xout))
...
class InvISPNet(nn.Module):
    def __init__(self, channel_in=4, subnet_constructor=subnet('DBNet'), block_num=4):
        super(InvISPNet, self).__init__()
        operations = []
        pan_operations = []
        # level = 3
        channel_num = channel_in * 2  # total channels at input stage
        ...
        for j in range(block_num):
            b = CoupleLayer(channel_num, subnet=PanProcess, substructor=subnet_constructor,
                            condition_length=4)  # one block is one flow step.
            operations.append(b)
    ...
    def forward(self, ms, pan, gt, rev=False):
        b, c, m, n = ms.shape
        _, _, M, N = pan.shape

        mHR = upsample(ms, M, N)

        # panlist = self.pyin.pyramid_decom(pan)
        # LapPan = panlist[0]
        # for i in range(1, len(panlist)):
        #     LapPan = torch.cat([LapPan, panlist[i]], dim=1)
        if not rev:
            x = mHR
            out = self.CG0(x)
            # out_list = []
...
#
#     def gauss_kernel(self, device=device, channels=1): #cuda
